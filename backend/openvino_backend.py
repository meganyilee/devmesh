...
def object_detection(self, uri, model, dev):
    ...
    obj_frame = cv2.cvtColor(np.array(pil_image), cv2.COLOR_BGR2RGB)
    ...
    if model.lower() == 'food' or model.lower() == 'gesture':
        ...
        return {'result': '', 'info': str(model) + 'not a valid model', 'status': 'completed'}
    ...
    if dev.lower() != 'cpu' and dev.lower() != 'myriad':
        return {'result': '', 'info': str(dev) + ' not a valid device', 'status': 'completed'}
    ...
    encoded_retval = encoded_retval.replace("'", "")
    return {'result': jpg_header + encoded_retval, 'info_state': preds[0], 'status': 'completed'}
